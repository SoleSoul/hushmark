...
The current GitHub release artifact is a Windows executable plus the normal source archives generated by GitHub for the tag. Linux runtime support ships in the same source tag; Linux desktop integration and install behavior belong to downstream packaging until a first-party Linux package is added. macOS app and DMG bundles are first-party build targets, but must not be published until the Universal bundle has been smoke-tested, signed with Developer ID, notarized, and stapled.
...
Build and smoke-test macOS releases on a Mac. Use `docs/macos-support.md` for the Finder, lifecycle, Intel/Apple Silicon, signing, and notarization checks.
...
Run the Universal build with Developer ID and notarization credentials for a public Mac artifact. An unsigned local DMG is suitable only for development testing.
...
- On macOS, complete every item in the Intel Mac smoke test in `docs/macos-support.md`; also test the same Universal app on Apple Silicon before publication.
