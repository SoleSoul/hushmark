export const PRODUCT = {
  displayName: "Hushmark",
  markdownExtensions: ".md and .markdown",
  setupTitle: "Hushmark Setup",
  installRowLabel: "Install Hushmark",
  updateRowLabel: "Update Hushmark",
  installRowDescription: "Copy or update Hushmark in your local Programs folder.",
  openWithRowLabel: "Show Hushmark in Open With",
  openWithRowDescription:
    "Let Windows offer Hushmark for .md and .markdown files. Installs Hushmark first if needed.",
  contextMenuRowLabel: "Add right-click menu entry",
  contextMenuRowDescription:
    "Add 'Open with Hushmark' to Markdown file context menus. Installs Hushmark first if needed.",
  chooseDefaultButtonLabel: "Choose Hushmark as default app...",
  removeAllButtonLabel: "Remove all Hushmark integration",
} as const;

